...
function ChatInput() {
  ...
  function sendImageMessage(image: string | ReadFileResult) {
    ...
    // TODO Can't upload pictures smaller than 3MB
    ...
    img.onload = async () => {
      ...
      try {
        ...
        handleSendMessage(id, 'image', `${imageUrl}?width=${img.width}&height=${img.height}`, focus)
      } catch (err) {
        console.error(err)
  ...
  async function handleInputKeyDown(e) {
    if (e.key === 'Tab') {
      ...
      // If you press @ build, enter @calculation mode
  ...
  function replaceAt(targetUsername) {
    $input.current.value = $input.current.value.replace(`@${at.content}`, `@${targetUsername} `)
    setAt({
      enable: false,
      content: ''
    })
      ...
      <div className={Style.chatInput} {...aero}>
          ...
          overlay={
            <div className={Style.featureDropdown}>
              <Menu onClick={handleFeatureMenuClick}>
                <MenuItem key="huaji">Random emoji</MenuItem>
        ...
        <form className={Style.form} autoComplete="off" onSubmit={e => e.preventDefault()}>
          <input
            ...
            type="text"
            placeholder="Just talk about something, do n’t make senseless screen"
            maxLength={2048}
            ref={$input}
