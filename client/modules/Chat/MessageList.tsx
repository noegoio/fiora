...
function MessageList() {
  ...
  async function handleScroll(e) {
    ...
    if ($div.scrollTop === 0 && $div.scrollHeight > $div.clientHeight) {
      ...
      if (isLogin) {
        historyMessages = await getLinkmanHistoryMessages(focus, Object.keys(messages).length)
      } else {
        historyMessages = await getDefalutGroupHistoryMessages(Object.keys(messages).length)
      }
