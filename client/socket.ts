...
socket.on('connect', async () => {
  ...
  if (token) {
    const user = await loginByToken(token, platform.os.family, platform.name, platform.description)
    if (user) {
      dispatch({
...
socket.on('message', async message => {
  ...
  if (linkman) {
    ...
    if (linkman.type === 'group') {
      title = `${message.from.username} in ${linkman.name} to everyone:`
    } else {
      title = `${message.from.username} says:`
    }
  } else {
    // Contact does not exist and is a message sent by yourself, no new contact is created
    if (isSelfMessage) {
      return
    }
    const newLinkman = {
    ...
    if (message.type === 'text') {
      ...
      const from =
        linkman && linkman.type === 'group'
          ? `${message.from.username}${linkman.name === prevName ? '' : `in ${linkman.name}`} says`
          : `${message.from.username} says`
      if (text) {
